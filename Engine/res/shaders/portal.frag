...
void main(){
    ...
    if(material.useTexture > 0) {
        ...
        //vec2 fc = vec2(gl_FragCoord.x/800, gl_FragCoord.y/600);

        vec4 texColor = clamp(texture(tex, tc), 0, 1); // Get base color from texture

        //vec4 texColor = clamp(texture(tex, gl_FragCoord.xy/(material.texEnd - material.texStart)), 0, 1);
        //vec4 texColor = clamp(texture(tex, vec2((gl_FragCoord.x/800.0) * (material.texEnd.x - material.texStart.x), (gl_FragCoord.y/600.0) * (material.texEnd.y - material.texStart.y))), 0, 1);
        //vec4 texColor = texture(tex, gl_FragCoord.xy);

        if((material.texEnd - material.texStart).x > 1)
        {
