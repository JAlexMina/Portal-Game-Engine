...
std::shared_ptr<Shape> ResourceLoader::readObj(const QString &path, std::shared_ptr<Shape> &shape) {
    // Open the file
    QFile file(path);
    ...
    while(!line.isNull())
    {
        ...
        else if(parts[0] == "f")
        {
            ...
            if(!success) {
                std::cout << "resourses failed" << std::endl;
                return shape;
            }
        }
        line = f.readLine().trimmed();
    ...
    // Return shape
    //std::cout << vertices.size() << " " << i << std::endl;
